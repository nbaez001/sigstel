 ...
 * @author NERIO
 */
public class AdministrarPerfilAdmController extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        if (request.getSession().getAttribute("administradorIdCuenta") != null) {
            int idCuentaAdministrador = (int) request.getSession().getAttribute("administradorIdCuenta");

            Usuario u = UsuarioBL.Obtener(idCuentaAdministrador);
            if (u != null) {
                Persona p = PersonaBL.Obtener(u.getIdPersona());

                request.getSession().setAttribute("cuentaAdministrador", u);
                request.getSession().setAttribute("persona", p);

                request.getRequestDispatcher("WEB-INF/jsp/adm/administrarPerfil.jsp").forward(request, response);
            }
        } else {
